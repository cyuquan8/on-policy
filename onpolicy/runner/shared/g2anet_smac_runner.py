...
from onpolicy.runner.shared.g2anet_base_runner import G2ANetRunner

def _t2n(x):
    return x.detach().cpu().numpy()

class G2ANetSMACRunner(G2ANetRunner):
    """Runner class to perform training, evaluation. and data collection for SMAC. See parent class for details."""
    def __init__(self, config):
        super(G2ANetSMACRunner, self).__init__(config)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                lstm_hidden_states_critic, lstm_cell_states_critic = self.collect(step)
            ...
            if episode % self.log_interval == 0:
                ...
                train_infos['dead_ratio'] = 1 - \
                    self.buffer.active_masks.sum() / reduce(lambda x, y: x*y, list(self.buffer.active_masks.shape)) 
    ...
    def collect(self, step):
        ...
        value, action, action_log_prob, lstm_hidden_states_actor, lstm_cell_states_actor, \
        lstm_hidden_states_critic, lstm_cell_states_critic \
    ...
    def insert(self, data):
        ...
        lstm_hidden_states_actor[dones_env == True] = \
            np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        lstm_cell_states_actor[dones_env == True] = \
            np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        lstm_hidden_states_critic[dones_env == True] = \
            np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        lstm_cell_states_critic[dones_env == True] = \
            np.zeros(((dones_env == True).sum(), self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
        ...
        self.buffer.insert(share_obs, obs, lstm_hidden_states_actor, lstm_cell_states_actor, \
    ...
    def eval(self, total_num_steps):
        ...
        eval_lstm_hidden_states_actor = \
            np.zeros((self.n_eval_rollout_threads, self.num_agents, self.recurrent_N, self.hidden_size), 
                     dtype=np.float32)
        ...
        while True:
            self.trainer.prep_rollout()
            eval_actions, eval_lstm_hidden_states_actor, eval_lstm_cell_states_actor = \
