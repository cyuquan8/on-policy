import torch
from onpolicy.algorithms.mudmaf_mappo.algorithm.mudmaf_actor_critic import MuDMAF_Actor, MuDMAF_Critic
from onpolicy.utils.util import update_linear_schedule


class MuDMAF_MAPPOPolicy:
    """
    MuDMAF MAPPO Policy class. Wraps actor and critic networks to compute actions and value function predictions.
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        self.actor = MuDMAF_Actor(args, self.obs_space, self.act_space, self.device)
        self.critic = MuDMAF_Critic(args, self.share_obs_space, self.device)
    ...
    def get_actions(self, cent_obs, obs, cent_goals, goals, lstm_hidden_states_actor, lstm_cell_states_actor, 
        ...
        :param cent_goals (np.ndarray): centralized goal input to the critic.
        :param goals (np.ndarray): local agent goal inputs to the actor.
    ...
    def get_values(self, cent_obs, cent_goals, lstm_hidden_states_critic, lstm_cell_states_critic, masks):
        """
        ...
        :return values: (torch.Tensor) value function predictions.
        """
        values, _, _ = self.critic(cent_obs, cent_goals, lstm_hidden_states_critic, lstm_cell_states_critic, masks)
        return values

    def evaluate_actions(self, cent_obs, obs, cent_goals, goals, lstm_hidden_states_actor, lstm_cell_states_actor, 
    ...
    def act(self, obs, goals, lstm_hidden_states_actor, lstm_cell_states_actor, masks, available_actions=None, 
            deterministic=False):
        """
