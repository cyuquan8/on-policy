...
class GATConv(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        # (num_nodes, in_channels) --> (num_nodes, att_heads, out_channels)
        ...
        # (num_nodes, att_heads, out_channels) --> (num_nodes, att_heads)
        ...
        # (num_nodes, att_heads, out_channels) 
    ...
    def edge_update(self, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        if edge_attr is not None and self.lin_edge is not None:
            ...
            # (num_edges, edge_dims) --> (num_edges, att_heads * out_channels)
            edge_attr = self.lin_edge(edge_attr)
            # (num_edges, att_heads * out_channels) -->
            # (num_edges, att_heads, out_channels)
            edge_attr = edge_attr.view(-1, self.heads, self.out_channels)
            # (num_edges, att_heads * out_channels) --> (num_edges, att_heads)
            alpha_edge = (edge_attr * self.att_edge).sum(dim=-1)
            # 'emulate' concatenation
            # (num_edges, att_heads) 
    ...
    def message(self, x_j: Tensor, alpha: Tensor) -> Tensor:
        # (num_edges, att_heads, 1) * (num_edges, att_heads, out_channels) -->
        # (num_edges, att_heads, out_channels)
        if self.gnn_cpa_model == 'none':
            # default implementation
            return alpha.unsqueeze(-1) * x_j
        elif self.gnn_cpa_model == 'f_additive':
            # add neighbours original feature vectors
            return (alpha.unsqueeze(-1) + 1) * x_j
