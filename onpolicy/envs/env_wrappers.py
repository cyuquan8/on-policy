...
def availableactionsgoalsworker(remote, parent_remote, env_fn_wrapper):
    parent_remote.close()
    env = env_fn_wrapper.x()
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, goal, reward, done, info, available_actions = env.step(data)
            if 'bool' in done.__class__.__name__:
                if done:
                    ob, goal, available_actions = env.reset()
            else:
                if np.all(done):
                    ob, goal, available_actions = env.reset()

            remote.send((ob, goal, reward, done, info, available_actions))
        elif cmd == 'reset':
            ob, goal, available_actions = env.reset()
            remote.send((ob, goal, available_actions))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'render':
...
class AvailableActionsGoalsSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        """
        envs: list of gym environments to run in subprocesses
        """
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=availableactionsgoalsworker, args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                   for (work_remote, remote, env_fn) in zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
            p.daemon = True  # if the main process crashes, we should not cause things to hang
            p.start()
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, goals, rews, dones, infos, available_actions = zip(*results)
        return np.stack(obs), np.stack(goals), np.stack(rews), np.stack(dones), infos, np.stack(available_actions)
    ...
    def reset(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        obs, available_actions = zip(*results)
        return np.stack(obs), np.stack(goals), np.stack(available_actions)
...
class AvailableActionsDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, rews, dones, infos, available_actions = map(
            np.array, zip(*results))
...
class AvailableActionsGoalsDummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns):
        self.envs = [fn() for fn in env_fns]
        env = self.envs[0]
        ShareVecEnv.__init__(self, len(
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, goals, rews, dones, infos, available_actions = map(
            np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], goals[i], available_actions[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    obs[i], goals[i], available_actions[i] = self.envs[i].reset()
        self.actions = None

        return obs, goals, rews, dones, infos, available_actions

    def reset(self):
        results = [env.reset() for env in self.envs]
        obs, goals, available_actions = map(np.array, zip(*results))
        return obs, goals, available_actions
