...
class SingleACTLayer(nn.Module):
    """
    MLP Module to compute actions for single agent (no shared weights)
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain):
        super(SingleACTLayer, self).__init__()
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        """
        # NOTE: ONLY DISCRETE (LAST ELSE CASE) IS FIXED FOR SINGLE AGENT
        if self.mixed_action:
            ...
            if active_masks is not None:
                dist_entropy = (action_logits.entropy()[active_masks.squeeze(-1).bool()]).mean()
            else:
